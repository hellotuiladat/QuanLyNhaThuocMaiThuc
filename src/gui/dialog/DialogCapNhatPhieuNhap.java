...
public class DialogCapNhatPhieuNhap extends JDialog {
    ...
    public DialogCapNhatPhieuNhap(Frame parent, FormPhieuNhapThuoc form, PhieuNhapThuoc pnh) {
        super(parent, "Cập nhật phiếu nhập thuốc", true);
        this.parentForm = form;
        this.phieuNhapHienTai = pnh;
    ...
    private void initComponents() {
        ...
        JLabel lblTitle = new JLabel("CẬP NHẬT PHIẾU NHẬP THUỐC");
        ...
        lblMaPhieuNhap.setFont(labelFont);
        formPanel.add(lblMaPhieuNhap, gbc);
        
        gbc.gridx = 1;
        gbc.weightx = 0.7;
        txtMaPhieuNhap = new JTextField();
        txtMaPhieuNhap.setFont(fieldFont);
        txtMaPhieuNhap.setPreferredSize(new Dimension(300, 35));
        txtMaPhieuNhap.setEnabled(false);
        txtMaPhieuNhap.setBackground(new Color(240, 240, 240));
        txtMaPhieuNhap.setBorder(BorderFactory.createCompoundBorder(
            new LineBorder(new Color(200, 200, 200), 1),
            new EmptyBorder(5, 10, 5, 10)
        ));
        formPanel.add(txtMaPhieuNhap, gbc);
        ...
        btnCapNhat.addActionListener(e -> xuLyCapNhatPhieuNhap());
    ...
    private void loadDataToForm() {
        try {
            txtMaPhieuNhap.setText(phieuNhapHienTai.getMaPhieuNhap());
            
            // Set Nhân viên
            if (phieuNhapHienTai.getNhanVien() != null) {
                String maNV = phieuNhapHienTai.getNhanVien().getMaNV();
                for (int i = 0; i < cboNhanVien.getItemCount(); i++) {
                    if (cboNhanVien.getItemAt(i).toString().startsWith(maNV)) {
            ...
            if (phieuNhapHienTai.getNhaCungCap() != null) {
                String maNCC = phieuNhapHienTai.getNhaCungCap().getMaNCC();
                for (int i = 0; i < cboNhaCungCap.getItemCount(); i++) {
                    if (cboNhaCungCap.getItemAt(i).toString().startsWith(maNCC)) {
            ...
            // Set Ngày nhập
            dateChooserNgayNhap.setDate(phieuNhapHienTai.getNgayNhap());
            
            // Load chi tiết phiếu nhập
            ArrayList<ChiTietNhapThuoc> chiTiets = pnhDAO.getChiTietPhieuNhap(phieuNhapHienTai.getMaPhieuNhap());
    ...
    private void themChiTiet() {
        ...
        try {
            ...
            ChiTietNhapThuoc ct = new ChiTietNhapThuoc(phieuNhapHienTai.getMaPhieuNhap(), maThuoc, soLuong, donGia);
            listChiTiet.add(ct);
    ...
    private void xuLyCapNhatPhieuNhap() {
        ...
        try {
            ...
            PhieuNhapThuoc pnh = new PhieuNhapThuoc(phieuNhapHienTai.getMaPhieuNhap(), nv, ncc, ngayNhap);
            
            boolean result = pnhDAO.capNhatPhieuNhapThuoc(pnh, listChiTiet);
            
            if (result) {
                JOptionPane.showMessageDialog(this, 
                    "Cập nhật phiếu nhập thuốc thành công!", 
                    "Thành công", 
                ...
                JOptionPane.showMessageDialog(this, 
                    "Cập nhật phiếu nhập thuốc thất bại!", 
                    "Lỗi", 
                    JOptionPane.ERROR_MESSAGE);
            }
